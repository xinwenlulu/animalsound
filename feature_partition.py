...
# Common folder was kindly provided by Georgios Rizos
...
def get_features_and_stats(waveform, orig_sr=16000, verbose=False):
    ...
    if verbose:
        if logmel_spectrogram.shape[0] != 500:
...
def make_folders():
    for partition in ["train", "devel", "test"]:
        ...
        positive_folder =  './Positives/' + partition + '/'
        negative_folder =  './Negatives/' + partition + '/'
        tfrecords_folder = './tfrecords/' + partition + '/'
...
def make_tfrecords(ann, nann, clipfolder, over3flist, sorted_species, random_generator=default_rng(), verbose = False):
    make_folders()
    positive_generator_list = list()
    negative_generator_list = list()

    unclipped_folder = clipfolder
    clipped_folder = './Positives'
    positive_extend = segmentcalls_fileindependent(unclipped_folder, clipped_folder, ann,
                                                   over3flist, sorted_species, random_generator = random_generator,
                                                   verbose = verbose)
    positive_generator_list.append(positive_extend)
    negative_extend = negcalls(unclipped_folder, './Negatives', nann, verbose=verbose)
    ...
    creator = TFRecordCreator(tf_records_folder='./tfrecords',
...
def segmentcalls_fileindependent(unclipped_folder, clipped_folder, anns, target, sorted_species,
    ...
    for i in index:
        ...
        for idx, time in enumerate(end_times):
            ...
            clip.export(clipped_folder + '/' + name + '.WAV', format="wav")
            waveform, sr = librosa.core.load(clipped_folder + '/' + name + '.WAV', sr=16000, duration=5.00)
...
def negcalls(unclipped_folder, clipped_folder, anns, verbose = False):
